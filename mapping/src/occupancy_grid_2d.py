...
class OccupancyGrid2d(object):
    ...
    def SensorCallback(self, msg):
        ...
        for idx, r in enumerate(msg.ranges):
            ...
            # TODO!
            obj_x, obj_y = sensor_x + r*np.cos(angle), sensor_y + r*np.sin(angle)
            x_step = (obj_x - sensor_x) / 100 
            x_ranges = np.arange(obj_x, sensor_x, -x_step)
            y_step = (obj_y - sensor_y) / 100 
            y_ranges = np.arange(obj_y, sensor_y, -y_step)
            # traverse backwards
            traversed_voxels = set()
            for x, y in zip(x_ranges, y_ranges):
                voxel = self.PointToVoxel(x, y)
                traversed_voxels.add(voxel)
            # update occupied voxel
            occupied_x, occupied_y = self.PointToVoxel(obj_x, obj_y)
            self._map[occupied_x, occupied_y] += self._occupied_update
            if self._map[occupied_x, occupied_y] >= self._occupied_threshold:
                self._map[occupied_x, occupied_y] = self._occupied_threshold
            # update free voxels
            for voxel in traversed_voxels:
                voxel_x, voxel_y = voxel
                # exclude occupied node
                if voxel_x == occupied_x and voxel_y == occupied_y:
                    continue
                # update free voxel
                self._map[voxel_x, voxel_y] += self._free_update
                if self._map[voxel_x, voxel_y] <= self._free_threshold:
                    self._map[voxel_x, voxel_y] = self._free_threshold
