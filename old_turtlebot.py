...
class Turtle:
    ...
    def __init__(self, objects):
        ...
        self.vel_publisher = rospy.Publisher('cmd_vel_mux/input/teleop', Twist, queue_size = 5)
        
        self.laser_subscriber = rospy.Subscriber('/scan', LaserScan, self.laser_callback, queue_size = 4)
    ...
    def rotate_mode(self):
        ...
        self.mileage = 0
        for i in range(80):
            self.publish_once_in_cmd_vel(0, 0.4)
            try:
                ...
                self.publish_once_in_cmd_vel(0, 0.3)
            if self.detected:
                list = [0, 0, 0]
...
class Following:
    ...
    def bounding_boxes_callback(self, boundbox_msg):
        ...
        if (self.ymax + self.ymin)/2 >= 405 and abs((self.xmin + self.xmax) / 2  - 320) <= 30:
            print('should exit whole program')
            nodes = os.popen("rosnode list").readlines()
            for i in range(len(nodes)):
                nodes[i] = nodes[i].replace("\n","")
            for node in nodes:
                if 'main_node' in node:
